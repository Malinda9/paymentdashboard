...
const ALLOWED = new Set([
  ...
  'admin_data_health','admin_save_payment','admin_delete_payment',
  'admin_teachers','admin_users','admin_save_user','admin_toggle_user',
  'admin_export_csv'
]);
...
module.exports = async function handler(req,res){
  try{
    ...
    if(!ALLOWED.has(action)){
      return res.status(400).json({success:false,message:'Invalid admin API action.'});
    }

    const url=process.env.APPS_SCRIPT_URL;
    if(!url){
      return res.status(500).json({success:false,message:'APPS_SCRIPT_URL is not configured.'});
    }
    ...
    if(action==='login'){
      ...
      if(!body.username||!body.password){
        return res.status(400).json({success:false,message:'Username និង Password ត្រូវបានទាមទារ។'});
      }
    }else{
      body.token=String(params.token||'').trim();
      if(!body.token){
        return res.status(401).json({success:false,message:'Session token មិនមាន។'});
      }

      for(const k of ['id','rowNumber','username','active','query']){
        if(params[k]!==undefined) body[k]=params[k];
      }

      for(const k of ['filters','student','payment','user']){
        if(params[k]!==undefined) body[k]=params[k];
      }
    }
